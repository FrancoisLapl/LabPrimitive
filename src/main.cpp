...
void LoopOverAllPixels(const IplImage *img, const IplImage *processed, float &fOrange, float &fWhite);
...
void ProcessImageBatch(int nbItems, char *character, FILE *fp, IplImage *&img, IplImage *&processed) {
    ...
    for (iNum = 1; iNum <= nbItems; iNum++) {
        BuildFileName(iNum, character, cFileName, true);
        ...
        LoopOverAllPixels(img, processed, fOrange, fWhite);
